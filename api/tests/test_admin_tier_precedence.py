"""Env deve ganhar de tiers.detalhes no limite de parlamentares."""
...
def test_env_wins_over_db_detalhes(monkeypatch: pytest.MonkeyPatch) -> None:
    # env diz 3; DB diz 7 -> deve valer 3.
    monkeypatch.setenv(
        "MAMUTE_TIER_LIMITS_JSON",
        '{"cidadao-mamute": {"qtd_termos": 3}}',
    )
    project = _FakeProject({"qtd_termos": 7})
    assert projects._legacy_global_favorite_limit(project) == 3


def test_db_detalhes_used_when_env_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("MAMUTE_TIER_LIMITS_JSON", raising=False)
    project = _FakeProject({"qtd_termos": 7})
    assert projects._legacy_global_favorite_limit(project) == 7


def test_column_fallback_when_both_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("MAMUTE_TIER_LIMITS_JSON", raising=False)
    project = _FakeProject({}, qtd_termos_col=2)
    assert projects._legacy_global_favorite_limit(project) == 2
